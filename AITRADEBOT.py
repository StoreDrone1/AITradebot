...
#from alpaca_trade_api import Alpaca
import time as t
import schedule
...
#you will have to input your api key / secret code respectively here
api = trade_api.REST('AK1WLH5VISIWR62IZQXC', 'vvq4JGnNtCDgZ4SQm2MUM9JaghKjiv91BTufKnFW')
...
try:
    model.fit(X_train, train_data['Close'], epochs=20, batch_size=64, verbose=2)
...
def trade():
    ...
    # get the latest data and update the training data
    ...
    #print("Predicted price of {} = {}".format(symbol, predictions[0]))
    if predictions[0] >= 0.07:
        # place a buy order
    ...
    with open("Predictions_Today.txt", 'a') as preds:
        preds.writelines(str(predictions[0]))
